const allServices = [
  ...
  },
  {
    ...
    subServices: ['Rekomendasi Sambung Baru Meter Pelanggan'],
  },

  // Pelayanan Publik pada Unit Pelayanan di BP Batam
  {
  ...
  },
  {
    ...
    subServices: ['Izin Pematangan Lahan', 'Izin Pemanfaatan ROW', 'Izin Reklame'],
  },
  {
...
const services: Service[] = allServices // menggunakan data yang sudah ada

// Fungsi helper untuk mendapatkan prefix antrian
function getQueuePrefix(service: string): string {
  return service
    .split(' ')
    .map((word) => word[0])
    .join('')
